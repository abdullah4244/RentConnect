      ...
      <header className={props.className}>
        <div className="flex md:flex-col flex-row md:gap-10 items-center justify-between w-full">
          ...
          <div className="flex flex-row gap-10 h-[42px] md:h-auto sm:hidden items-center justify-start w-[228px]">
            ...
            <Button onClick={onLoginClick} className="bg-gray-900 cursor-pointer font-manrope font-semibold py-2.5 rounded-[10px] text-base text-center text-white-A700 w-full">
              Log in
            </Button>
          </div>
        </div>
      </header>
      <LogInModal isOpen={isLoginModalOpen} onRequestClose={()=>setIsLoginModalOpen(false)} onCreateAccount={onSignupClick}/>
      <CreateAccountModal isOpen={isRegisterModal} onRequestClose={()=>setIsRegisterModal(false)}/>
    </>
  );
};
